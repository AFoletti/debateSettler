/**
 * DebateSettler Dashboard - Enhanced with Trends View
 * Clean version with single "Trends" option
 */
...
// Check if current aggregation is trends view
...
function convertToLegacyFormat(aggregatedData) {
    ...
    // Handle aggregated format (weekly, monthly, trends)
    return {
        billable_hours: aggregatedData.billable_hours?.sum || 0,
...
function processRawDataLegacy(rawData) {
    ...
    // Calculate metrics for 30 days (fallback only)
    ...
    const dailyBillableAvg = last30WorkingDays.length > 0 ? billableHours / last30WorkingDays.length : 0;

    return {
        billable_hours: Math.round(billableHours * 100) / 100,
        daily_billable_avg: Math.round(dailyBillableAvg * 100) / 100,
        absent_from_home_hours: 0,
        daily_away_avg: 0,
        back_home_stats: { count: 0, mean: 'N/A', median: 'N/A', earliest: 'N/A', latest: 'N/A' },
        home_office_end_stats: { count: 0, mean: 'N/A', median: 'N/A', earliest: 'N/A', latest: 'N/A' },
        late_work_frequency: { late_work_days: 0, total_work_days: last30WorkingDays.length, percentage: 0 },
